...
// 	1 способ ( через стили)
// 	const btns = document.querySelectorAll(triggersSelector),
// 			blocks = document.querySelectorAll(itemsSelector);
// 	blocks.forEach(block => {
// 		block.classList.add('animated', 'fadeInDown');
// 	});
// 	// Используя этот метод, мы в стили main.css добавляем display none на все блоки изначально, и display block на active,
// 	// и стили в active-style для выделения текста активного блока(906, 913, 917)
// 	btns.forEach(btn => {
...
var accordion = function accordion(triggersSelector) {
  // 2 способ ( через высоту блока)
  // Тут мы также добавляем стили в main.css 917 928 
  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      btns.forEach(function (btn) {
        // Закрывает все другие блоки контента при открытие активного
...
var burger = function burger(menuSelector, burgerSelector) {
  ...
  window.addEventListener('resize', function () {
    // если окно изменяется 
    if (window.screen.availWidth > 992) {
      // становится выше 992 по ширине
      menuElem.style.display = 'none'; // то скрываем открытое бургер меню
    }
  });
};
...
var drop = function drop() {
  // drag *
  // dragend *
  // gragenter - объект над dropArea
  // dragexit *
  // dragleave - объект за пределами dropArea
  // dragover - объект зависает над dropArea
  // dragstart
  // drop - объект отправлен в dropArea
  var fileInputs = document.querySelectorAll('[name="upload"]'); // наш инпут куда будем перетаскивать картинку

  ['dragenter', 'dragleave', 'dragover', 'drop'].forEach(function (eventName) {
    // массив событий, перебираем
  ...
  function preventDefaults(e) {
    // сбрасываем обычное поведение
    e.preventDefault();
    e.stopPropagation();
  }

  function highlight(item) {
    item.closest('.file_upload').style.border = '5px solid yellow'; // ближайший элемент к (file_upload) задаем параметр
  ...
  function unhighlight(item) {
    ...
    if (item.closest('.calc_form')) {
      // если есть родитель с таким классом то
      item.closest('.file_upload').style.backgroundColor = '#fff'; // цвет фона будет белый ( обычная форма)
    } else {
      item.closest('.file_upload').style.backgroundColor = '#ededed'; // иначе серый ( форма которая вылазиет)
    }
  }

  ['dragenter', 'dragover'].forEach(function (eventName) {
    // при наведении мышки с картинкой в область дропа будет применяться ф-ия которая его покрасит(выделит)
    fileInputs.forEach(function (input) {
  ...
  ['dragleave', 'drop'].forEach(function (eventName) {
    // уберет выделение если область мыши с картинкой вне дропа
    fileInputs.forEach(function (input) {
      input.addEventListener(eventName, function () {
  ...
  fileInputs.forEach(function (input) {
    input.addEventListener('drop', function (e) {
      // обработчик на сбрасывание элемента в область
      input.files = e.dataTransfer.files; // dataTransfer - объект с файлом засовываем в наш инпут

      var dots; // это и все ниже обрезает название перетаскиваемой картинки

      var arr = input.files[0].name.split('.'); // получаем название картинки и делим название на 2 части, до точки и после точки

      arr[0].length > 6 ? dots = "..." : dots = '.'; // мы тут как бы разделям название файла картинки до точки[0] и после точки[1] и говорим что если будет больше 5 символов то будет подставляться ... если же нет то .

      var name = arr[0].substring(0, 6) + dots + arr[1]; // вырезаем название от 0 до 6 + точки( либо ... либо 1) + расширение png jpg и тд
...
var forms = function forms() {
  ...
  var message = {
    succses: 'Спасибо! Мы скоро с вами свяжимся',
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var dots;
      var arr = item.files[0].name.split('.'); // получаем название картинки и делим название на 2 части, до точки и после точки
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var api;
      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question; // указываем куда будем отправлять
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    // pos это кол-во символов в this.val
    elem.focus();
  ...
  function createMask(event) {
    ...
    def = matrix.replace(/\D/g, ''),
        // работает на основе матрицы, статичная
    val = this.value.replace(/\D/g, ''); // динамичная, то что вводит пользователь

    if (def.length >= val.length) {
      // не дает пользователяю удалять +7 и тд, нашу форму
      val = def;
    }

    this.value = matrix.replace(/./g, function (a) {
      // то что вводи пользователь, заменяем все символы на ->
      return /[_\d]/.test(a) && i < val.length ? val.charAt(i++) : i >= val.length ? '' : a; // [диапозон].совпадения(каждый перебираемый символ внутри матрицы)
    });
...
var modals = function modals() {
  ...
  document.querySelector('.fixed-gift').addEventListener('mouseenter', function (e) {
    e.target.classList.add('animated', 'animate__tada');
  });
  document.querySelector('.fixed-gift').addEventListener('mouseleave', function (e) {
    e.target.classList.remove('animated', 'animate__tada');
  });
...
var scrolling = function scrolling(upSelector) {
  ...
  window.addEventListener('scroll', function () {
    if (document.documentElement.scrollTop > 1650) {
      // появление скрола после 1650 px от начала
  ...
  }); // Pure Js Scrolling (1 метод)
  // const element = document.documentElement,	// эти две переменные исп для старых и новых браузеров
  // 		body = document.body;
  // const calcScroll = () => {
  // 	upElem.addEventListener('click', function(e) {
  // 		let scrollTop = Math.round(body.scrollTop || element.scrollTop);	// какое расстояние было пролистано вниз // то есть тут мы говорим что если есть это или это(одно из них используется в старых браузерах) то есть мини адаптация под старые браузеры
  // 		if (this.hash !== '') {		// #up - это hash, тут говорится, если хэш не пустой, то есть есть
  // 			e.preventDefault();
  // 			let hashElement = document.querySelector(this.hash),
  // 			 	hashElementTop = 0;
  // 			while (hashElement.offsetParent) {			// элемент, относительно которого позиционируется hashElement
  // 				hashElementTop += hashElement.offsetTop;		// // сколько пикселей осталось до верхней границы родительского элемента
  ...
  // };
  // calcScroll();
  // Scrolling with Request Animation frame ( 2 метод )

  var links = document.querySelectorAll('[href^="#"]'),
      // ищет все локальные ссылки
  speed = 0.3;
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    // исп.обычную ф-ию чтобы this.remove() заработала, контекс вызова
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('http://localhost:3000/styles') // также можно вставить путь к файлу ->  assets/db.json 
    .then(function (res) {
      return createCards(res);
    }) // и вместо res -> res.styles (styles это с db.json)
  ...
  function createCards(response) {
    response.forEach(function (_ref) {
      var src = _ref.src,
          title = _ref.title,
          link = _ref.link;
      // деструктуризация(вытаскиваем переменные с бэкэнда(db.json))
...
/* harmony default export */ __webpack_exports__["default"] = (showMoreStyles); // Другой способ(без взаимодействия с сервером)
